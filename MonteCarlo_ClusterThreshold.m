% function [] = MonteCarlo_ClusterThreshold(x_matrix, y_matrix, slices, dim_xy, dim_z, FWHM, dim_resampled, ...
%                                      mask_name, mask_bytes, mask_plot, p_corrected, p_voxel, iterations, savename)
%
% MonteCarlo_ClusterThreshold written by Scott D. Slotnick 
%
...
%                                      mask_name, mask_bytes, mask_plot, p_corrected, p_voxel, iterations, savename)
%
% Example, MonteCarlo_ClusterThreshold(64,64,30,4.5,4.5,0,3,'none',0,0,.05,.01,1000,'ctb_ofm_pc05_pv01') 

% Obleser et al. (2011, NI)
% 64 x 64 matrix size (acquisition matrix)
% 26 slices
% 3 x 3 mm inplane resolution
% 3 mm slice thickness (with counting the gap)
% 8 mm FWHM
% 2 x 2 x 2 mm after resampling
% 'none', 0, 0 = no mask
% p = .05 at cluster level
% p = .001 at voxel level
% 10 iterations
% save as 'test.mat'
% MonteCarlo_ClusterThreshold(64,64,26,3,3,8,2,'none',0,0,.05,.001,10,'test'); %Obleser et al., 2011, NI

% Bianco et al (2020)
% 70 x 70 matrix size (acquisition matrix)
% 37 slices
% 3 x 3 mm inplane resolution
% 3.68 mm slice thickness (with counting the gap)
% 8 mm FWHM
% 2 x 2 x 2 mm after resampling
% 'none', 0, 0 = no mask
% p = .05 at cluster level
% p = .001 at voxel level
% 10 iterations
% save as 'MonteCarlo_p001_p05.mat'
%MonteCarlo_ClusterThreshold(70,70,37,3,3.68,8,2,'none',0,0,.05,.001,10,'MonteCarlo_p001_p05_10iterations');

function [] = MonteCarlo_ClusterThreshold(x_matrix, y_matrix, slices, dim_xy, dim_z, FWHM, dim_resampled, mask_name, mask_bytes, mask_plot, p_corrected, p_voxel, iterations, savename)
search_step = 0.1;
